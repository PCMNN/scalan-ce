...
!!!("Cannot convert from AbstractVector to ShiftVector: missing fields List(offset)")
  ...
  // 4) constructor and deconstructor
  class ShiftVectorCompanionAbs extends CompanionDef[ShiftVectorCompanionAbs] with ShiftVectorCompanion {
...
!!!("Cannot convert from AbstractVector to ShiftVectorBoxed: missing fields List(offset)")
  ...
  // 4) constructor and deconstructor
  class ShiftVectorBoxedCompanionAbs extends CompanionDef[ShiftVectorBoxedCompanionAbs] with ShiftVectorBoxedCompanion {
...
trait VectorsSeq extends scalan.ScalanDslStd with VectorsDsl {
  ...
  def mkShiftVector[T]
    ...
    new SeqShiftVector[T](nonZeroIndices, nonZeroValues, offset, length)
...
trait VectorsExp extends scalan.ScalanDslExp with VectorsDsl {
  ...
  object DenseVectorCompanionMethods {
    ...
    object apply {
      def unapply(d: Def[_]): Option[(Coll[Int], Coll[T], IntRep) forSome {type T}] = d match {
        case MethodCall(receiver, method, Seq(nonZeroIndices, nonZeroValues, length, _*), _) if receiver.elem == DenseVectorCompanionElem && method.getName == "apply" && method.getAnnotation(classOf[scalan.OverloadId]) == null =>
    ...
    object apply_DenseVectorCompanion_apply_nonZeroItems {
      def unapply(d: Def[_]): Option[(Coll[(Int, T)], IntRep) forSome {type T}] = d match {
        case MethodCall(receiver, method, Seq(nonZeroItems, length, _*), _) if receiver.elem == DenseVectorCompanionElem && method.getName == "apply" && { val ann = method.getAnnotation(classOf[scalan.OverloadId]); ann != null && ann.value == "DenseVectorCompanion_apply_nonZeroItems" } =>
          Some((nonZeroItems, length)).asInstanceOf[Option[(Coll[(Int, T)], IntRep) forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Coll[(Int, T)], IntRep) forSome {type T}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
  ...
  object SparseVectorCompanionMethods {
    object apply {
      def unapply(d: Def[_]): Option[Coll[T] forSome {type T}] = d match {
        case MethodCall(receiver, method, Seq(items, _*), _) if receiver.elem == SparseVectorCompanionElem && method.getName == "apply" && method.getAnnotation(classOf[scalan.OverloadId]) == null =>
          Some(items).asInstanceOf[Option[Coll[T] forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[Coll[T] forSome {type T}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
    ...
    object apply_SparseVectorCompanion_apply_nonZeroItems {
      def unapply(d: Def[_]): Option[(Coll[(Int, T)], IntRep) forSome {type T}] = d match {
        case MethodCall(receiver, method, Seq(nonZeroItems, length, _*), _) if receiver.elem == SparseVectorCompanionElem && method.getName == "apply" && { val ann = method.getAnnotation(classOf[scalan.OverloadId]); ann != null && ann.value == "SparseVectorCompanion_apply_nonZeroItems" } =>
          Some((nonZeroItems, length)).asInstanceOf[Option[(Coll[(Int, T)], IntRep) forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Coll[(Int, T)], IntRep) forSome {type T}] = exp match {
  ...
  object SparseVectorBoxedCompanionMethods {
    object apply {
      def unapply(d: Def[_]): Option[Coll[T] forSome {type T}] = d match {
        case MethodCall(receiver, method, Seq(items, _*), _) if receiver.elem == SparseVectorBoxedCompanionElem && method.getName == "apply" && method.getAnnotation(classOf[scalan.OverloadId]) == null =>
          Some(items).asInstanceOf[Option[Coll[T] forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[Coll[T] forSome {type T}] = exp match {
    ...
    object apply_SparseVectorBoxedCompanion_apply_nonZeroItems {
      def unapply(d: Def[_]): Option[(Coll[Int], Coll[T], IntRep) forSome {type T}] = d match {
        case MethodCall(receiver, method, Seq(indices, values, length, _*), _) if receiver.elem == SparseVectorBoxedCompanionElem && method.getName == "apply" && { val ann = method.getAnnotation(classOf[scalan.OverloadId]); ann != null && ann.value == "SparseVectorBoxedCompanion_apply_nonZeroItems" } =>
          Some((indices, values, length)).asInstanceOf[Option[(Coll[Int], Coll[T], IntRep) forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Coll[Int], Coll[T], IntRep) forSome {type T}] = exp match {
        case Def(d) => unapply(d)
  ...
  object ShiftVectorCompanionMethods {
    object zero {
      def unapply(d: Def[_]): Option[IntRep forSome {type T}] = d match {
        case MethodCall(receiver, method, Seq(len, _*), _) if receiver.elem == ShiftVectorCompanionElem && method.getName == "zero" =>
          Some(len).asInstanceOf[Option[IntRep forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[IntRep forSome {type T}] = exp match {
  ...
  object ShiftVectorBoxedCompanionMethods {
    object zero {
      def unapply(d: Def[_]): Option[IntRep forSome {type T}] = d match {
        case MethodCall(receiver, method, Seq(len, _*), _) if receiver.elem == ShiftVectorBoxedCompanionElem && method.getName == "zero" =>
          Some(len).asInstanceOf[Option[IntRep forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[IntRep forSome {type T}] = exp match {
...
object Vectors_Module extends scalan.ModuleInfo {
  val dump = ""
}
}
