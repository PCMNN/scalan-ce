...
trait ArrayOpsExp extends ArrayOps with BaseExp { self: ScalanExp =>
  ...
  case class GenericInnerJoin[A, B, K, R](xs: Exp[Array[A]], ys: Exp[Array[B]], a: Rep[A => K], b: Rep[B => K],
                                          f: Rep[((A, B)) => R])
                                         (implicit ordK: Ordering[K], nK: Numeric[K], selfType: Elem[Array[(K, R)]], eA: Elem[A], eB: Elem[B], eK: Elem[K], eR: Elem[R]) extends ArrayDef[(K, R)]

  case class GenericOuterJoin[A, B, K, R](xs: Exp[Array[A]], ys: Exp[Array[B]], a: Rep[A => K], b: Rep[B => K],
